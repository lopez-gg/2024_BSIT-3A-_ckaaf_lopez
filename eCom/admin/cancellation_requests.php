  ...
  <div class="product-display">
    <table class="product-display-table order-table">
      ...
      <?php
        ...
        $query = "SELECT * FROM orders WHERE cancel_request = 'Pending'
        ...
        if ($result->num_rows > 0) {
          ...
          while ($rows = $result->fetch_assoc()) {
            ...
            $cancel_status = htmlspecialchars($rows['cancel_request']);
            ...
            if ($orderId !== $currentOrderId) {
              ...
              echo '<tr class="order-group ' . $rowClass . '"><td colspan="9">Order Reference Number: ' . $orderId . '</td><td>';
              echo '<form id="orderForm' . $orderId . '" class="orderForm" action="../request/update_order_status.php" method="post">';
              echo '<input type="hidden" name="orderId" value="' . $orderId . '">';
              echo '<select class="statbtn" name="cancel_status" id="cancel_status' . $orderId . '">';
              echo '<option value="Pending"' . ($orderStatus == 'Pending' ? ' selected' : '') . '>Pending</option>';
              echo '<option value="Approve"' . ($orderStatus == 'Approve' ? ' selected' : '') . '>Aprove</option>';
              echo '<option value="Reject"' . ($orderStatus == 'Reject' ? ' selected' : '') . '>Reject</option>';
              echo '</select>';
